...
def get_dataset(name, sparse=True, feat_str="deg+ak3+reall", root=None):
    if root is None or root == 'D:/Python/GithubCode/AutoGCL-master/data':
    ...
    if 'MNIST' in name or 'CIFAR' in name:
        ...
        #     use_node_attr=True)
        # 正确：将root设置为用户指定的data_root（如data），name设置为数据集名称（如MUTAG）
        dataset = TUDataset(
            root=root,  # 直接使用用户指定的data_root（如"D:/.../data"）
            name=name,  # 数据集名称（如"MUTAG"）
            pre_transform=pre_transform,
            use_node_attr=True
        )
